...
class SemanticSearchManager:
    def __init__(self):
        ...
        self.index = None
        self.productos = {}  # Dict para acceso rápido por ID
        self.corpus = {}     # Dict que mapea ID -> texto
        self.id_to_faiss_idx = {}  # Mapea ID de producto -> índice en FAISS
        self.faiss_idx_to_id = {}  # Mapea índice en FAISS -> ID de producto
        self.dimension = 768  # Dimensión del modelo all-mpnet-base-v2
        ...
        # Cargar datos existentes si hay un backup
        self._load_backup()
        
        # Si no hay backup, cargar desde BD
    ...
    def _obtener_producto_desde_mysql(self, producto_id: int) -> Optional[Dict]:
        """Obtiene un producto específico desde MySQL"""
    ...
    def _obtener_todos_productos_desde_mysql(self) -> List[Dict]:
        """Obtiene todos los productos desde MySQL"""
        connection = self._get_db_connection()
        if not connection:
            return []
    ...
    def _crear_texto_producto(self, producto: Dict) -> str:
        """Crea el texto de búsqueda para un producto"""
    ...
    def _save_backup(self):
        """Guarda backup del estado actual"""
        try:
            backup_data = {
    ...
    def agregar_producto(self, producto_id: int) -> bool:
        """Agrega un nuevo producto al índice"""
        try:
            ...
            with self.lock:
                # Si el producto ya existe, actualizarlo en su lugar
                ...
                # Guardar backup de forma asíncrona
                self._save_backup()
                return True
    ...
    def actualizar_producto(self, producto_id: int) -> bool:
        """Actualiza un producto existente en el índice"""
        try:
            ...
            producto = self._obtener_producto_desde_mysql(producto_id)
            if not producto:
                # Si no se encuentra en la BD, eliminarlo del índice
                return self.eliminar_producto(producto_id)
            
            with self.lock:
                # Obtener índice FAISS del producto
                faiss_idx = self.id_to_faiss_idx[producto_id]
                
                # Crear nuevo texto y embedding
                nuevo_texto = self._crear_texto_producto(producto)
                nuevo_embedding = self.model.encode([nuevo_texto], normalize_embeddings=True)
                
                # FAISS no permite actualizar directamente, así que necesitamos reconstruir
                # Para evitar reconstruir todo, marcamos para reconstrucción lazy
                self.productos[producto_id] = producto
                ...
                for fid in range(self.next_faiss_idx):
                    if fid in self.faiss_idx_to_id:
                        ...
                        if pid in self.corpus:
                            if pid == producto_id:
                                # Usar el nuevo embedding
                                all_embeddings.append(nuevo_embedding[0])
                            else:
                                # Re-encodificar el texto existente
